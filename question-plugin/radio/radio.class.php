...
class radioClass extends question{
	...
	Public function layout($field, $xml=false){
			...
			if(isset($field['row'])){
				foreach($field['row'] AS $key=>$row){
					if($xml){
						...
						$rows .= ' <li style="'.$height.'" ><input type="radio" name="q'.$field['question_id'].'[]" id="row'.$key.'" class="row" value="'.$row['question_row'].'" ><label  for="row'.$key.'">'.$row['row_label'].'</label></li>'."\n";
					}
				}
			}
			...
			if($xml){
				...
				<question>
					...
					<class>".$field['question_type']."</class>
					<input>
						<id type=\"radio\">q".$field['question_id']."</id>
				...
				$ret = '<li id="qid_'.$field['question_id'].'" class="'.$field['question_type'].'" style="line-height: 1.5em">
                 	<div class="question-label">'.$field['question_label'].'</div>
                   	<div class="question-block" style="'.$text.'">
                        <ul id="q'.$field['question_id'].'" style="'.$width.';line-height: 1.5em" class="radio">
                      	'.$rows.'
                      </ul>
