$(document).ready(function(){
	...
	$("#title-field").live('keyup', function(e){
		$('.title').html($('#title-field').val());
		if($('.title').html() == ''){
			$(".title").warning("There is no Title, you will need a Title before you can save this Survey");
		}
	});

	$("#details-field").live('keyup', function(e){
		$('.details').html($('#details-field').val());
		if($('.details').html() == ''){
...
function pageSetProp(id){
			var pattern = /^<div class="warning"\>/;
			
			//disregard content if it is a warning
			var title = ($('.title').html().match(pattern))? '' : $('.title').text();
			var details = ($('.details').html().match(pattern))? '' : $('.details').text();
			
			$('#title-field').val(title);
			$('#details-field').val(details);
			...
			$("#page-font").slider({value:labelfont,
								...
								slide: function( event, ui ) {
									...
									$(".question-block td").css({"font-size":ui.value+'px', 'line-height': '105%'});
									$(".question-block th").css({"font-size":ui.value+'px', 'line-height': '105%'});
